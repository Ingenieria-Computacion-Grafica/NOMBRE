...
//sol = new ESFERA(gl,0.9f,100,100,-2,10f,25,1f,1f,0,1,1,1,0,0,0);
    ...
    public ESFERA( float x, float y, float z,float w, float h, float d,float rx, float ry, float rz, float r, float g, float b,Texture tex,GL gl, float ra, int slices, int stacks) {
    ...
    GLU glu=new GLU();
if(tex!=null){
//    System.out.println("Activando textura");
...
//    glu.gluQuadricDrawStyle (esfera,GLU.GLU_SILHOUETTE);
   ...
   if(this.t!=null){
    glu.gluQuadricTexture(esfer, true);
}
    glu.gluSphere(esfer,ra,slices,stacks);//recibe el radio, numero de cortes horizontales y verticales
